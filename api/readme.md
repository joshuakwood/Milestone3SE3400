...
Change Filter | PUT | /users/website-settings | website:(str), new_filter_settings:(str in json format) | If the user is logged in and filter (website) exists, this changes filter settings for a specific webiste on the users account. example: {"ads": 1,"cookies": 1,"paywall": 0,"bias-source": 1,"cyber_safety": 0,"subscription": 0,"family_friendly": 1} 
